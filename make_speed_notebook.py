...
cells = [
    ...
    md(
        ...
        "`speed_benchmark.csv`.\n"
        "\n"
        "**`MAX_SIDE` matters:** GNHK photos are ~4000 px, and PP-OCRv5 *server* on full-size images can\n"
        "exhaust Colab RAM (it OOMs during warmup). Capping the longest side to ~2000 px cuts peak memory\n"
        "~4× — both systems get the **same** resized input, so the comparison stays fair. Lower it further\n"
        "(or set `BASELINE_TIER='mobile'` in §4) if you still crash."
    ),
    code(
        "import cv2\n"
        "from pathlib import Path\n"
        ...
        "MAX_SIDE = 2000             # downscale longest side so PP-OCRv5 server fits in RAM (0 = off)\n"
    ...
    code(
        ...
        "BASELINE_TIER = 'server'   # 'mobile' if server crashes / OOMs\n"
        "RUN_BASELINE = True\n"
        "\n"
        ...
        "\n"
        "# free HI-RES (TrOCR) before loading the heavier PP-OCRv5 server pipeline -> lower peak memory\n"
        "del ht; gc.collect()\n"
        ...
        "\n"
        "print('\\n' + B.report(hires, paddle, device, BEAMS, WARMUP,\n"
