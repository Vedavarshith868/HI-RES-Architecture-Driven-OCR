...
cells = [
    ...
    md("## 1. Install dependencies (~2 min)\n"
       "Pins the exact stack that runs PaddleOCR on GPU (T4/cu126). "
       "**Runtime → Change runtime type → T4 GPU first.**"),
    code(
        "# GPU-verified stack: paddlepaddle-gpu 3.3.1 + paddleocr 3.7.0 + paddlex 3.7.1\n"
        "# (plain `pip install paddleocr` pulls cpu-only paddle and runs on CPU)\n"
        ...
        "     '-f', PADDLE_IDX])\n"
        "# write constraint so paddleocr/paddlex can't downgrade paddle\n"
        "open('/tmp/constraints.txt', 'w').write('paddlepaddle-gpu==3.3.1\\n')\n"
        "run([sys.executable, '-m', 'pip', 'install', '-q',\n"
        "     'paddleocr==3.7.0', 'paddlex==3.7.1',\n"
        "     '-c', '/tmp/constraints.txt'])\n"
        "run([sys.executable, '-m', 'pip', 'install', '-q',\n"
        "     '--force-reinstall', '--no-deps',\n"
        "     'paddlepaddle-gpu==3.3.1', '-f', PADDLE_IDX])\n"
        "run([sys.executable, '-m', 'pip', 'install', '-q',\n"
        "     'transformers>=4.45', 'safetensors', 'datasets'])\n"
        "\n"
        "import torch, paddle, paddleocr, transformers\n"
        "print('torch', torch.__version__, '| cuda:', torch.cuda.is_available())\n"
        "print('paddle device:', paddle.device.get_device(),\n"
        "      '| paddleocr', paddleocr.__version__)\n"
        "print('transformers', transformers.__version__)\n"
        "# Must print gpu:0 — if still cpu, re-run from a fresh T4 runtime"
    ),
    ...
    code(
        ...
        "BASELINE = 'v6-medium'  # PP-OCRv6 medium — same det as HI-RES, fair comparison\n"
        "RUN_BASELINE = True\n"
        "DET_MODEL = 'PP-OCRv6_medium_det'  # HI-RES uses same det as baseline tier\n"
        "\n"
        "paths = B.list_images(IMAGES_DIR)\n"
        "imgs = [B.read_rgb(p) for p in paths]\n"
        "print(f'{len(imgs)} pages from {IMAGES_DIR}')\n"
        "\n"
        "ht = B.HiResTimer(beams=BEAMS, det_model=DET_MODEL)\n"
        "device = ht.device\n"
    ...
    code(
        ...
        "ax.bar('HI-RES', det, label='detect (PP-OCRv6 medium)')\n"
