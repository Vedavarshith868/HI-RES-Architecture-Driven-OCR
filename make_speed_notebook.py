...
cells = [
    ...
    md(
        "## 3. Build test pages\n"
        "By default this synthesizes **real-handwriting** multi-line pages by stacking IAM lines with a\n"
        "small skew (auto-downloaded from Hugging Face — no manual upload). To benchmark on **your own**\n"
        "pages instead, set `SOURCE = 'upload'` and pick image files when prompted.\n"
        "\n"
        "Speed is independent of ground truth, so any representative full pages work."
    ),
    code(
        "import cv2\n"
        "from pathlib import Path\n"
        ...
        "\n"
        "SOURCE = 'synth'        # 'synth' = IAM pages (auto) | 'upload' = pick your own images\n"
