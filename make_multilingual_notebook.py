...
cells = [
    ...
    md(
        "## 5. Clean forms: HI-RES vs stock PaddleOCR (controlled, same PP-OCRv6 models)\n"
        "Both run the **same** PP-OCRv6 det+rec, so the only differences are HI-RES's reading order,\n"
        "keep-every-box, and batched recognition. On clean upright forms expect **~parity on CER** (naive\n"
        "order is already correct) with HI-RES **faster** (it batches recognition across the whole page).\n"
