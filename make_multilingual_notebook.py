...
cells = [
    ...
    md(
        ...
        "\n"
        "**`CONTROLLED = True` matters for a fair test.** It gives stock PaddleOCR the *same* det+rec models\n"
        "as HI-RES, so the only differences are HI-RES's reading order, keep-every-box, and crop method —\n"
        "that isolates the layer. With `False`, stock uses PaddleOCR's heavier **default** models, so any\n"
        "speed/accuracy gap is mostly the model tier (mobile vs server), *not* the architecture — an\n"
    ...
    code(
        ...
        "norm = E.NormCfg()\n"
        "\n"
        "CONTROLLED = True   # stock uses the SAME det+rec as HI-RES (fair test of the reading-order layer)\n"
        "stock_det, stock_rec = (DET_MODEL, REC_MODEL) if CONTROLLED else (None, None)\n"
        "\n"
        "wm = not no_space   # CJK has no word boundaries -> WER/WordAcc are meaningless, skip them\n"
        "scores = [E.evaluate_system(f'hires-ml[{PPOCR_LANG}]', samples,\n"
        "                            M.hires_predict(engine, no_space),\n"
        "                            norm=norm, progress=True, word_metrics=wm)]\n"
        "try:\n"
        "    scores.append(E.evaluate_system(f'paddle-stock[{PPOCR_LANG}]', samples,\n"
        "                                    M.builtin_predict(PPOCR_LANG, no_space, stock_det, stock_rec),\n"
        "                                    norm=norm, progress=True, word_metrics=wm))\n"
